...
Scenario('Showing empty liked movies', ({ I }) => {
  ...
  // I.saveScreenshot('liked-movie.png');
});
...
Scenario('searching movies', async ({ I }) => {
  ...
  // mendapatkan daftar film yg sesuai dgn searchQuery
  const matchingMovies = titles.filter((title) => title.indexOf(searchQuery) !== -1);
  const visibleSearchedLikeMovies = await I.grabNumberOfVisibleElements('.movie-item');

  assert.strictEqual(matchingMovies.length, visibleSearchedLikeMovies);
